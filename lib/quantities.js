var Qty = (function() {
  ...
  constructor.prototype = {
    parse: function(val) {
      ...
      var top = result[2];
      console.log("topmatch: " + top);
      var unit_match;
      this.numerator = [];
      while((unit_match = UNIT_MATCH_REGEX.exec(top))) {
        console.log("topunitmatch: " + unit_match);
        this.numerator.push(unit_match.slice(1));
      }
      //console.log("numerator: ");
      //console.log(this.numerator);
      var bottom = result[3];
      console.log("bottommatch: " + bottom);
      while((unit_match = UNIT_MATCH_REGEX.exec(bottom))) {
        console.log("bottomunitmatch: " + unit_match);
        this.denominator = unit_match.slice(1);
      }
      ...
      this.numerator = (function() {
        console.log(this.numerator);
        var numerator = [];
        this.numerator.forEach(function(item) {
          numerator.push(PREFIX_MAP[item[0]] ? [PREFIX_MAP[item[0]], UNIT_MAP[item[1]]] : [UNIT_MAP[item[1]]]);
        });
        return numerator;
      }).call(this);
      console.log("numerator: ");
