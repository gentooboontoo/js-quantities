(function() {
  ...
  */
  ...
  function parse(val) {
    ...
    // Scan
    if(top) {
      if(!(new RegExp(UNIT_TEST_REGEX).test(top))) {
        throw "Unit not recognized";
      }
      this.numerator = [];
    ...
    if(bottom) {
      if(!(new RegExp(UNIT_TEST_REGEX).test(bottom))) {
        throw "Unit not recognized";
      }
      this.denominator = [];
  ...
  window.Qty.prototype = {
    ...
    */
    ...
    toPrec: function(prec_quantity) {
      ...
      var result = Math.round(this.scalar/prec_quantity.scalar)*prec_quantity.scalar;

      // Remove potential floating error based on prec_quantity exponent
      var prec_exponent = Math.floor(Math.log(prec_quantity.scalar) / Math.LN10);
      result = Math.round(result * Math.pow(10, -prec_exponent)) / Math.pow(10, -prec_exponent);

      return new Qty(result + this.units());
    },
