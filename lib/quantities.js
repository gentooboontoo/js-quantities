var Qty = (function() {
  ...
  var constructor = function(init_value) {
    this.scalar = null;
  ...
  function parse(val) {
    ...
    while((result = TOP_REGEX.exec(top))) {
      ...
      if(n >= 0) {
        ...
        top = top.replace(new RegExp(result[1] + "(\\^|\\*\\*)" + n), "");
      }
    }
  ...
  constructor.prototype = {
    ...
    */
    ...
    units: function() {
      ...
      this.unit_name = on.join("*") + ((output_d == UNITY_ARRAY) ? '':'/' + od.join("*")); 
      return this.unit_name;
    },
    ...
    toString: function(target_units) {
      if(this.output[target_units]) {
        return this.output[target_units];
      } 

      var out = target_units ? this.to(target_units) : this;
      out = (out.scalar + " " + out.units()).trim();
    ...
    to: function(other) {
      ...
      var q = this.base_scalar / target.base_scalar;
    ...
    mul: function(other) {
      ...
      other = other.to(this);
      var numden = cleanTerms(this.numerator.concat(other.numerator), this.denominator.concat(other.denominator));

      return new Qty({"scalar": this.scalar * other.to(this).scalar, "numerator": numden[0], "denominator": numden[1]});
    },
    ...
    div: function(other) {
      ...
      other = other.to(this);
      var numden = cleanTerms(this.numerator.concat(other.denominator), this.denominator.concat(other.numerator));

      return new Qty({"scalar": this.scalar / other.scalar, "numerator": numden[0], "denominator": numden[1]});
    }
