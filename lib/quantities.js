  ...
  */
  ...
  constructor.prototype = {
    ...
    */
    ...
    isBase: function() {
      ...
      if(this.signature == 400 && this.numerator.length == 1 && this.denominator == UNITY_ARRAY && this.units.match(/(deg|temp)K/)) {
        this.is_base = true;
        return this.is_base;
      }
    ...
    toBase: function() {
      ...
      // TODO cache
      /*var cached = base_unit_cache[this.units] * this.scalar;
      if(cached) {
        return cached;
      }*/
      ...
      // TODO mult() + cache
      base.scalar *= this.scalar;
      return base;
    },

    // returns the 'unit' part of the Unit object without the scalar
    units: function() {
      if(this.numerator == UNITY_ARRAY && this.denominator == UNITY_ARRAY) {
        return "";
      }
      if(this.unit_name) {
      ...
      if(this.numerator == UNITY_ARRAY) {
        output_n = UNITY_ARRAY;
      }
      else {
        for(var i = 0; i < this.numerator.length; i++) {
          var token = this.numerator[i];
          var token_next = this.numerator[i+1];
      ...
      if(this.denominator == UNITY_ARRAY) {
        output_d = UNITY_ARRAY;
      }
      else {
        for(var i = 0; i < this.denominator.length; i++) {
          var token = this.denominator[i];
          var token_next = this.denominator[i+1];
          if(PREFIX_VALUES[token]) {
      ...
      var out = on.join("*") + ((output_d == UNITY_ARRAY) ? '':'/' + od.join("*")); 
      return out;
    },
  ...
  function unique(duplicates) {
    return duplicates.reduce(function(previous, current, index, array) {
      if(!previous.some(function(item) {return compare(item, current);})) {
